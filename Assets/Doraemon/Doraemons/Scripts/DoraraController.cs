...
public class DoraraController : MonoBehaviour
{
        ...
        else
        {
            ...
            if (inputForward != lastForward)
            {
                ...
                else
                {
                    ...
                    indexWP++;
                    cornerIndex.Enqueue(indexWP);
                    Debug.LogWarning("SetWP/" + indexWP + "/Frame: " + Time.frameCount + "/Pos: " + myTransform.position.ToString("F2"));
                }
            }
    ...
    public void Follow(bool move, DoraraController followee, int index)
    {
        if (isStart)
        {
            ...
            if (softLayer == 1)
            {
                int cindex = followee.cornerIndex.Dequeue();
                Debug.LogWarning("GetWP/" + cindex + "/Frame: " + Time.frameCount + "/Pos: " + waypoint.ToString("F2") + "/" + myTransform.position.ToString("F2"));
            }
            ...
            if (!follow)
            {
                do
                {
                    ...
                    if (followee.cornerPos.Count > 0)
                    {
                        ...
                        {
                            follow = false;
                            //Debug.LogWarning("GetWP/Same/Frame: " + Time.frameCount + "/Pos: " + waypoint.ToString("F2") + "/" + myTransform.position.ToString("F2"));
                        }
                    }
                    else
                    {
                        ...
                        //if (softLayer == 1)
                        //    Debug.LogWarning("XP/" + softLayer + "/Frame: " + Time.frameCount + "/Pos: " + waypoint.ToString("F2") + "/" + myTransform.position.ToString("F2") + "/" + name);
                    }
                }
                while (!follow);
            }
        ...
        Move();
        //if (isMoving && softLayer == 1)
        //    Debug.Log(softLayer + "/" + myTransform.position.ToString("F2") + "/" + followee.myTransform.position.ToString("F2") + "/" + waypoint.ToString("F2"));
    }
